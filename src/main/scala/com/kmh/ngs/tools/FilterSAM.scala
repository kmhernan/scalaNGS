...
//import com.kmh.ngs.readers.
//import com.kmh.ngs.cmdline.
...
/**
 * Filters multi-mapped SAM records based on user-input 
 * 
 ...
 */
class MultiMapSAM(val args: List[String]) extends NGSApp with Logging {
  def toolName = "'%s'".format(this.getClass()) 
  def description = "Provides statistics on multi-mapped reads based on user inputs."
  def mainUsage = List(
